...
class Asteroid {
  ...
  constructor(x: number, y: number, size: 1 | 2 | 3 = 3) {
    this.x = x;
    this.y = y;
    this.size = size;
  ...
  split(): Asteroid[] {
    ...
    const nextSize = (this.size - 1) as 1 | 2;
    return [
...
class Ship {
  ...
  update(dt: number, input: AsteroidesInput) {
    if (this.dead) return;
...
export class AsteroidesEngine {
  ...
  private lives = 3;
  private level = 1;
  private status: AsteroidesState["status"] = "playing";
  private deadTimer = 0;
  ...
  update(dt: number, input: AsteroidesInput) {
    const shootJustPressed = input.shoot && !this.prevShoot;
  ...
  getState(): AsteroidesState {
    return {
      ...
      lives: this.lives,
      level: this.level,
      tripleShotRemaining: Math.max(this.ship.tripleShot, 0),
    };
  }
}
