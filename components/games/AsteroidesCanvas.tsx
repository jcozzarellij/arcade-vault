...
type AsteroidesCanvasProps = {
  paused: boolean;
  onStateChange: (state: AsteroidesState) => void;
  onGameOver: (finalScore: number) => void;
  restartSignal: number;
};

const KEY_MAP: Record<string, keyof AsteroidesInput> = {
  ArrowLeft: "left",
  ArrowRight: "right",
  ArrowUp: "thrust",
  Space: "shoot",
...
export default function AsteroidesCanvas({
  paused,
  onStateChange,
  onGameOver,
  restartSignal,
  ...
  useEffect(() => {
    ...
    const engine = new AsteroidesEngine();
    engineRef.current = engine;
    gameOverFiredRef.current = false;

    const input: AsteroidesInput = {
      left: false,
      right: false,
      thrust: false,
      shoot: false,
